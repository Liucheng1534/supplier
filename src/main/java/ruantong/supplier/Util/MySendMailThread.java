...
public class MySendMailThread extends Thread {
	...
	public void run() {
		...
		try {
			...
			// 3邮件内容:主题、内容(这里是重点)
			msg.setSubject("尊敬的用户您好" + ",欢迎您使用本网站,请确认验证码");

			// StringBuilder是线程不安全的,但是速度快，这里因为只会有这个线程来访问，所以可以用这个
			StringBuilder sbd = new StringBuilder();
			sbd.append("尊敬的用户您好" + "<br/>欢迎！请确认此邮件地址以确认您的账号。<br/>");
			sbd.append("<font color='red'><a href='"
					+ "'");
			sbd.append(">验证码:"+user.getUsername() +"</a></font><br/>");
			sbd.append("非常感谢您使用本网站！<br/>");
